...
struct match_result : std::optional<detail::forwarder<T>> {
  ...
  decltype(auto) get() && { return (*static_cast<super &&>(*this)).forward(); }
};
  ...
  if constexpr (is_identifier_v<Decayed>) {
    if constexpr (Decayed::has_pattern) {
      ...
      constexpr std::size_t i =
          find_indexed_forwarder_v<Decayed::index,
...
constexpr auto parse(StrView str_view) {
  ...
  if constexpr (I == std::string_view::npos) {
    ...
    constexpr std::size_t next =
        comma == std::string_view::npos ? std::string_view::npos : comma + 1;
...
auto try_match(const Identifier<I, Pattern> &identifier, Value &&value, F &&f) {
  ...
  if constexpr (Identifier<I, Pattern>::has_pattern) {
    return try_match(identifier.arg, std::forward<Value>(value), std::move(f_));
  } else {
    return f_();
  }
}
      ...
      [&](auto &&...head_ifs) {
        ...
        return try_match_impl(
            ...
            [&](auto &&...tail_ifs) {
              ...
              return match_invoke(std::forward<F>(f), std::move(head_ifs)...,
                                  std::move(tail_ifs)...);
            },
...
auto expand_variadics(const Ds<Patterns...> &p,
  ...
  if constexpr (variadic_index == npos) {
    ...
    if constexpr (num_values < num_patterns) {
      ...
      return ds(std::get<(variadic_index <= Is ? Is + 1 : Is)>(p.patterns)...);
    } else {
      static_assert(num_values >= num_patterns);
...
public:
  ...
  decltype(auto) operator()(Case<Pattern, Rhs> &&case_, Cases &&...cases) && {
    auto result = [&] {
