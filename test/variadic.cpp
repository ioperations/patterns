...
template <typename F, typename Tuple> decltype(auto) apply(F &&f, Tuple &&t) {
  ...
  return match(std::forward<Tuple>(t))(pattern(ds(variadic(arg))) =
                                           std::forward<F>(f));
}
...
template <typename F, typename... Vs> decltype(auto) visit(F &&f, Vs &&...vs) {
  using namespace mpark::patterns;
  return match(std::forward<Vs>(vs)...)(pattern(variadic(vis(arg))) =
                                            std::forward<F>(f));
}
...
TEST(Variadic, Middle) {
  ...
  int result = match(tuple)(
      ...
      pattern(ds(x, variadic(arg), x)) =
          [](auto, auto... args) {
            static_assert(sizeof...(args) == 2);
            return 2;
          },
      pattern(ds(x, y, y, variadic(arg), x)) =
          [](auto, auto, auto... args) {
