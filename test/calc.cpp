...
private:
  ...
  template <std::size_t I, typename FwdBinary,
            std::enable_if_t<std::is_base_of_v<Binary, std::decay_t<FwdBinary>>,
                             int> = 0>
  friend auto &&get(FwdBinary &&binary) noexcept {
    if constexpr (I == 0)
      return *std::forward<FwdBinary>(binary).lhs_;
    if constexpr (I == 1)
      return *std::forward<FwdBinary>(binary).rhs_;
  }
};

class Func : public Unary {
public:
  using Unary::Unary;
};
...
std::ostream &operator<<(std::ostream &strm, const Expr &expr) {
  ...
  match(expr)(
      ...
      pattern(as<Plus>(ds(lhs, rhs))) =
          [&](auto &&lhs, auto &&rhs) {
            strm << "(+ " << lhs << ' ' << rhs << ')';
          },
      pattern(as<Mult>(ds(lhs, rhs))) =
          [&](auto &&lhs, auto &&rhs) {
            strm << "(* " << lhs << ' ' << rhs << ')';
          },
      pattern(as<Func>(ds(arg))) =
          [&](auto &&body) { strm << "(fn [] " << body << ')'; });
  return strm;
}
...
int eval(const Expr &expr) {
  ...
  return match(expr)(
      ...
      pattern(as<Plus>(ds(lhs, rhs))) =
          [](auto &&lhs, auto &&rhs) { return eval(lhs) + eval(rhs); },
      pattern(as<Mult>(ds(lhs, rhs))) =
          [](auto &&lhs, auto &&rhs) { return eval(lhs) * eval(rhs); },
