...
template <>
class tuple_size<N::S> : public integral_constant<size_t, 2> {};
template <>
class tuple_element<0, N::S> {
   public:
    using type = int;
};
template <>
class tuple_element<1, N::S> {
   public:
    using type = string;
};
...
TEST(Destructure, Custom) {
    ...
    int result = match(s)(
        ...
        pattern(arg(ds(101, arg))) =
            [](const auto &x, const auto &y) {
                static_assert(
                    std::is_same<N::S, std::decay_t<decltype(x)>>::value, "");
                static_assert(
