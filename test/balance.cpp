...
template <typename T> struct part_ptr : std::unique_ptr<T> {
...
template <typename T, typename... Args> auto make_part(Args &&...args) {
...
template <typename Arg> std::string stringify(Arg &&arg) {
...
template <typename T> class Node {
public:
  explicit Node(Color color, memory::part_ptr<Node> &&lhs, T t,
                memory::part_ptr<Node> &&rhs)
      : color_(color), lhs_(std::move(lhs)), t_(std::move(t)),
        rhs_(std::move(rhs)) {}
...
private:
  ...
  template <std::size_t I, typename FwdNode,
            std::enable_if_t<is_node<std::decay_t<FwdNode>>::value, int> = 0>
  friend auto &&get(FwdNode &&node) noexcept {
    if constexpr (I == 0)
      return std::forward<FwdNode>(node).color_;
    else if constexpr (I == 1)
      return std::forward<FwdNode>(node).lhs_;
    else if constexpr (I == 2)
      return std::forward<FwdNode>(node).t_;
    else if constexpr (I == 3)
      return std::forward<FwdNode>(node).rhs_;
  }
};

template <typename T> void Node<T>::balance() {
  ...
  match(std::move(*this))(
      pattern(
          ...
          [&](auto &&a, auto &&x, auto &&b, auto &&y, auto &&c, auto &&z,
              auto &&d) {
#define FWD(x) std::forward<decltype(x)>(x)
            *this = Node(
                Red, memory::make_part<Node>(Black, FWD(a), FWD(x), FWD(b)), y,
...
template <typename T> class tuple_element<0, rb::Node<T>> {
public:
  using type = rb::Color;
};

template <typename T> class tuple_element<1, rb::Node<T>> {
public:
  using type = memory::part_ptr<rb::Node<T>>;
};

template <typename T> class tuple_element<2, rb::Node<T>> {
public:
  using type = T;
};

template <typename T> class tuple_element<3, rb::Node<T>> {
public:
  using type = memory::part_ptr<rb::Node<T>>;
};
...
TEST(Balance, Typical) {
  auto tree = memory::make_part<rb::Node<int>>(
      ...
      memory::make_part<rb::Node<int>>(
          rb::Red,
          memory::make_part<rb::Node<int>>(rb::Red, nullptr, 101, nullptr), 202,
          nullptr),
      303, nullptr);
