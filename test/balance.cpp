...
void Node<T>::balance() {
    ...
    match(std::move(*this))(
        pattern(anyof(
            ds(Black, some(ds(Red, some(ds(Red, a, x, b)), y, c)), z, d),
...
#define FWD(x) std::forward<decltype(x)>(x)
                *this = Node(
                    Red, memory::make_part<Node>(Black, FWD(a), FWD(x), FWD(b)),
                    y, memory::make_part<Node>(Black, FWD(c), FWD(z), FWD(d)));
#undef FWD
            },
        pattern(_) = [] {});
}
...
template <typename T>
class tuple_element<0, rb::Node<T>> {
   public:
    using type = rb::Color;
};
...
template <typename T>
class tuple_element<2, rb::Node<T>> {
   public:
    using type = T;
};
