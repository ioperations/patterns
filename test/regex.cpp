...
TEST(Regex, String) {
  auto lex = [](std::string_view sv) {
    ...
    return match(sv)(
        pattern(arg(re.match(R"~([_a-zA-Z]\w*)~"))) =
            [](auto lexeme) { return Token(Token::ID, std::string(lexeme)); },
        pattern(arg(re.match(R"~(-?\d+)~"))) =
            [](auto lexeme) { return Token(Token::NUM, std::string(lexeme)); },
        pattern(arg(re.match(R"~([*|/|+|-])~"))) =
            [](auto lexeme) { return Token(Token::OP, std::string(lexeme)); });
  };
...
TEST(Regex, Stream) {
  ...
  std::vector<Token> expected = {
      {Token::ID, "foo"},  {Token::OP, "+"}, {Token::NUM, "-42"},
      {Token::OP, "-"},    {Token::ID, "x"}, {Token::OP, "*"},
      {Token::NUM, "101"}, {Token::OP, "/"}, {Token::ID, "bar"}};

  std::vector<Token> actual;
  for (std::string token; strm >> token;) {
    using namespace mpark::patterns;
    actual.push_back(match(token)(
        pattern(arg(re.match(id))) =
            [](const auto &lexeme) { return Token(Token::ID, lexeme); },
        pattern(arg(re.match(num))) =
            [](const auto &lexeme) { return Token(Token::NUM, lexeme); },
        pattern(arg(re.match(op))) =
            [](const auto &lexeme) { return Token(Token::OP, lexeme); }));
  }
